...
namespace basecross
{
	...
	void PNTDXModelDraw::UpdateAnimation(float elapsedTime)
	{
		m_currentAnimeTime += elapsedTime * m_speed;
		UpdatePose();
	}
	...
	void PNTDXModelDraw::UpdatePose()
	{
		...
		if (m_currentClipIndex >= 0 && m_currentClipIndex < static_cast<int>(m_clips.size()))
		{
			...
			timeTicks = WrapTimeTicks(timeTicks, anim.duration, true);
		}
		else
		{
			timeTicks = m_currentAnimeTime * ticksPerSecond;
			timeTicks = WrapTimeTicks(timeTicks, anim.duration, m_loop);
		}
